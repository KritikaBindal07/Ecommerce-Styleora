...
const createUser = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const resetURL = `
        ...
        <b>Hey,</b>
        <b> Here's your OTP to log into your Styleora account.${otp}</b>
        </div>
        `;
    const data = {
      to: email,
...
const removeFromCart = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const index = cart.products.findIndex((product) => product.equals(id));
...
const applyCoupan = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    let totalAfterDiscount = Math.floor(
      cartTotal - (cartTotal * validCoupan.discount) / 100
    );
